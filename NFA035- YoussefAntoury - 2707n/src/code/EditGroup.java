...
public class EditGroup extends JFrame implements ActionListener {
	...
	JLabel listTitle = new JLabel(" Update Group");
	...
	EditGroup(Group g,GroupFrame f){
		parentF = f;
		selectedg = g;
		setSize (500, 650);
	...
	gNameBar.setText(g.getGrpName());
	descriptionBar.setText(g.getGrpDescription());
	...
	public void createTable(Group g) {
		int j =0;
		for(int i=0; i<Contact.getContactList().size(); i++) {
			...
			rowList[0]=name;
			rowList[1]=cityName;
			rowList[2]=g.getGrpContacts().contains(Contact.getContactList().get(i));
	...
	public void save() {
		...
		if( gNameBar.getText().equals("") ) {
			...
			selectedg.setGrpName(gNameBar.getText().trim()); selectedg.setGrpDescription(descriptionBar.getText().trim()); 
		for(int i=0; i<Contact.getContactList().size();i++) {
			
			if((Boolean)contactTableModel.getValueAt(i, 2)==true && selectedg.getGrpContacts().contains(Contact.getContactList().get(i))==false) {
				System.out.println((String) contactTableModel.getValueAt(0, 0) +(String) contactTableModel.getValueAt(1, 1));
				selectedg.addContact(Contact.getContactList().get(i));
			}

			if((Boolean)contactTableModel.getValueAt(i, 2)==false && selectedg.getGrpContacts().contains(Contact.getContactList().get(i))==true) {
				System.out.println((String) contactTableModel.getValueAt(0, 0) +(String) contactTableModel.getValueAt(1, 1));
				selectedg.removeContact(Contact.getContactList().get(i));
			}
		...
		parentF.updateGrpTable();
			//GroupFrame.updateGroupTable(parentF);
			
		parentF.showContacts(selectedg);
			//stream 
